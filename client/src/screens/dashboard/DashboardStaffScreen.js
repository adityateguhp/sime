...
import { FETCH_PROJECTS_QUERY, FETCH_TASKS_CREATEDBY_QUERY, FETCH_ASSIGNED_TASKS_QUERY_BYSTAFF, FETCH_PICS_BYSTAFF_QUERY } from '../../util/graphql';
...
const DashboardStaffScreen = ({ navigation }) => {
  ...
  const { data: picsByStaff, error: error4, loading: loading4, refetch: refetchPicStaff } = useQuery(
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={loading1 && loading2 && loading3 && loading4}
          onRefresh={onRefresh} />
      }
    >
      ...
      <Surface style={styles.containerRecent}>
        ...
        </View>
        {
          projectsStaff.length !== 0 ?
            <View style={{ marginBottom: projectsStaff.length === 1 ? 10 : 0 }}>
              <ProjectCorousel projects={projectsStaff} loading={loading1} selectItemHandler={selectItemHandler} />
            </View>
            :
